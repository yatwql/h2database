...
> **本章导读**: 本章为希望深入源码学习的读者提供实用的开发指南，包括高效的源码阅读路线、调试环境搭建方法和 H2 测试框架的使用说明。
> **前置知识**: 第1-10章全部内容（本章是全书的实践引导篇）
> **章节要点**:
> - 掌握 H2 源码的阅读路线和最佳实践
> - 学会搭建 H2 源码的调试和开发环境
> - 了解 H2 的测试框架和编写测试的方法
...
如图 11-1 所示，详见第3章《核心包结构详解》、第6章《H2 数据库核心算法分析》和第9章《持久化引擎深度解析》。

**图 11-1: 概览 H2 源码包从 JDBC 到 MVStore 的依赖链**
...
如图 11-2 所示，以下流程图展示了 SELECT 查询从 SQL 字符串到返回结果集的完整生命周期：

**图 11-2: 追踪 SELECT 从 JDBC 到存储层的完整路径**
...
如图 11-3 所示，理解 MVStore 在磁盘上的物理布局有助于调试和性能分析：

**图 11-3: 拆解 MVStore 文件的 Chunk 与 Page 布局**
...
如图 11-4 所示，如图 11-3 所示，H2 源码体积适中但架构完整，建议按照"宏观 → SQL → 存储 → 高级"的递进顺序阅读，每个阶段建立在前一个阶段的理解之上。各阶段的详细背景可参阅对应章节：阶段1（宏观理解）对应第1-2章架构分层和第3章包结构，阶段2（SQL 执行）对应第4章前半部分的命令与表达式模块，阶段3（存储引擎）对应第4章后半部分的 MVStore 模块和第6章核心算法，阶段4（高级主题）对应第7-8章查询优化和第10章锁与并发控制。

**图 11-4: 串联 H2 源码阅读的四阶段递进路线**
...
1. 从 `Engine.java` 和 `Database.java` 理解全局生命周期
2. 通过 `Constants.java` 和 `DbSettings.java` 掌握系统配置
3. 借助 `Mode.java` 了解 SQL 兼容模式
...
4. 跟踪 `JdbcStatement.java` → `Command.java` → `CommandContainer.java` 的 JDBC 到命令链路
5. 解析 `Parser.java` 和 `Tokenizer.java` 的 SQL 处理逻辑
6. 研究 `Select.java` 和 `Query.java` 的 SELECT 执行流程
...
如图 11-5 所示，**阅读策略**：这些主题相对独立，可按兴趣选择性阅读。`Optimizer.java` 比了解实现更重要的是理解代价模型的设计思路。`CacheLongKeyLIRS.java` 是 LIRS 算法的工业实现，可与经典 LRU 对比理解其优势。`MVRTreeMap.java` 展示了如何在 B-Tree 架构上叠加 R-Tree 空间索引。

**图 11-5: 罗列四个阅读阶段的前置要求与预期收获**

```text
============================================================

  如图 11-6 所示，阶段 1 ──── 前置 ────┬─── 收获 ────
                        │
...
**图 11-8: 拆解 H2 Console 从浏览器到引擎的调用层**
...
**图 11-9: 追踪 SQL 从断点入口到存储层的完整生命周期**
...
如图 11-10 所示，H2 支持通过 `org.h2.Driver.trace` 开启 JDBC 追踪日志。也可以通过 `DbSettings` 设置 `traceLevel` 为 1-4 分别对应 ERROR、INFO、DEBUG、TRACE 级别，输出详细执行日志。

> 以上调试方法均可在 IDE 中通过条件断点、求值表达式和热替换等标准功能辅助完成。详细的 IDE 操作可参考各 IDE 官方文档，此处不再赘述。

**图 11-10: 罗列 H2 八处关键调试入口的断点位置**
...
**图 11-11: 拆解 IDE 断点调试 H2 内核的三步流程**
...
掌握第 11 章的文件索引、阅读路线与调试入口后，下列四题用来检验读者能否把这些"导读资源"转化为可执行的源码探索动作。

**1. 🟢★ 描述 §11.1.4 中 `Query`、`Select` 与聚合相关类（如 `Aggregate`）之间的继承与协作关系，并指出查询执行时谁先被调用。**

> 提示：注意 `Query` 是 SELECT/UNION 的公共基类，`Aggregate` 属于表达式而非命令；调用顺序与 prepare/execute 阶段相关。
> 回顾：§11.1.4、§11.1.6

**2. 🟢★ 复述 §11.3.1 测试金字塔中四层（基准 / 单元 / 集成 / E2E）的覆盖范围，并各举一个 H2 中对应的测试类作为例子。**

> 提示：从 `TestMVStore`、`TestMVCC`、`TestServer`、`Benchmark` 等命名规律入手。
> 回顾：§11.3.1

**3. 🔵★★ 若希望用 IDE 完整观察一次事务提交，应当在哪些类的哪些方法上设置断点，才能依次看到 SQL 入口、事务状态切换、Chunk 写入与 fsync？**

> 提示：参考 §11.3.3 的断点速查表，注意 `TransactionCommand`、`TransactionStore.commit`、`MVStore.store`、`MVStore.sync` 的层次关系。
> 回顾：§11.3.3、§11.3.4

**4. 🟠★★★ 在本地按照 §11.2 第一阶段步骤搭建 H2 调试环境，对一条简单的 SELECT 设置 §11.3.3 的三个断点，并记录每个断点处看到的关键变量。**

> 提示：选择 `jdbc:h2:mem:test` 内存模式简化环境；在 `JdbcStatement.executeQuery`、`Optimizer.optimize`、`MVMap.get` 三处停下后用 Evaluate Expression 观察 `sql`、`bestPlan`、`rootReference`。
> 回顾：§11.2、§11.3.3

## 11.6 延展阅读

- H2 官方文档《Tutorial》(`h2/src/docsrc/html/tutorial.html`) — 从零开始的构建与测试指南
- H2 GitHub 仓库 (`https://github.com/h2database/h2database`) — 源码浏览与 issue 追踪
- 本书第1章《总体架构》 — 宏观架构与六组分层视图
- 本书第2章《分层模块划分》 — 各层模块的职责边界
- 本书第3章《核心包结构详解》 — 源码包的类层次全景

---

# 第12章 总结

> **本章导读**: 本章对全书内容进行总结，归纳 H2 的设计哲学、适用场景和技术特点，并通过与 SQLite、Derby、HSQLDB 等的多维度对比，帮助读者理解 H2 在嵌入式数据库领域的独特定位。
> **前置知识**: 全书第1-11章
> **章节要点**:
> - 回顾 H2 的设计哲学和核心特点
> - 理解 H2 在嵌入式数据库领域的定位和优劣势
> - 了解不同场景下数据库选型的考量因素
> - 获得进一步学习 H2 和数据库技术的建议
> **术语参考**: 本章涉及的专业术语详见书末[术语表](back/glossary.md)。
...
如图 12-1 所示，### 12.1.1 权衡一：COW B-Tree vs In-place B-Tree
...
**图 12-1: 对比 COW 与 In-place B-Tree 写入差异**
...
如图 12-2 所示，4. **空间回收**：旧版本数据不会被立即删除。当不再需要旧版本时（事务结束、无活跃事务引用），通过 Compaction 过程回收空间。这需要额外的 GC 机制。

**图 12-2: 刻画 COW B-Tree 写放大随时间的变化曲线**
...
如图 12-3 所示，### 12.1.2 权衡二：Log-structured + Chunk vs Traditional Page Store
...
4. **Compaction 策略**：MVStore 的 Compaction 选择"存活率低"的 Chunk 合并。存活率 = Chunk 中仍在使用的数据比例。当一个 Chunk 中大部分数据已经更新或删除（即成为"垃圾"），Compaction 将该 Chunk 中剩余的存活数据读取出来，写入新的 Chunk，然后回收原 Chunk 的空间。这与 Java GC 中的"复制收集器"有相似之处。
...
如图 12-4 所示，6. **恢复时间**：Log-Structured 存储崩溃恢复只需要扫描 Chunk 列表找到最后一个有效根引用，通常可以在毫秒级完成。传统 Page Store 需要重放 WAL，恢复时间与 WAL 大小成正比。

**图 12-4: 拆解 MVStore Compaction 的生命周期阶段**
...
如图 12-5 所示，### 12.1.3 权衡三：Single Writer CAS vs Read-Write Locks
...
H2 的分层选择代表了"可维护性优先"的设计哲学——在嵌入式数据库场景中，代码清晰度比极致性能更重要。
...
以下三张图分别从覆盖范围、调用链路与横向对比三个角度刻画 H2 的安全与综合能力：图 12-14 拆解六层安全防护的覆盖范围，图 12-15 追踪安全机制的关键调用链路，图 12-7 对比 H2 与三大嵌入式数据库的多维能力评分。

**图 12-14: 拆解 H2 六层安全防护的覆盖范围**
...
**图 12-15: 追踪 H2 安全机制的关键调用链路**

H2 提供了多层安全防护（详见官方文档 `security.html` 和 `advanced.html` 中的 SQL 注入、远程访问和安全协议章节）：
...
如图 12-8 所示，| 特性 | H2 | SQLite | Derby | HSQLDB |
|------|-----|--------|-------|--------|
...
**图 12-8: 对比 H2 SQLite Derby HSQLDB 四层架构**
...
**事务隔离**：H2 提供完整的 5 级隔离支持，包括 Oracle 风格的多版本读一致性（Snapshot Isolation）和 Serializable。SQLite 因为写操作互斥，只有 Serializable 级别可用——在高隔离级别下通过串行化写操作实现。Derby 默认使用 Read Committed，也支持 Repeatable Read 和 Serializable，但不提供 Read Uncommitted 和 Snapshot Isolation。HSQLDB 提供多级隔离支持，但不同隔离级别的实现细节与 H2 有所不同。
...
**网络协议**：H2 独有地支持 PostgreSQL Wire Protocol（PgServer），使得 PostgreSQL 客户端可以直接连接到 H2。同时提供 JDBC 原生协议和 Web Console，是协议支持最丰富的嵌入式数据库。SQLite 是嵌入式 C 库，不直接支持网络协议——需要借助第三方封装（如 sql.js、LiteStream）。Derby 支持 JDBC 网络服务端模式，HSQLDB 支持 JDBC 和 HTTP 服务，但两者都不支持 PG Wire 协议。
...
如图 12-10 所示，- **架构演进的历史痕迹**：H2 从 1.x 的 PageStore 引擎演进到 2.x 的 MVStore 引擎（详见第9章《持久化引擎深度解析》和第10章《锁实现与并发控制》），源码中保留了清晰的版本分界和接口抽象。通过对比新旧引擎的实现，可以深入理解数据库存储引擎的演进逻辑和设计取舍——比如为什么从 page-based 转向 log-structured，为什么引入 COW B-Tree 替代 in-place 更新。

**图 12-10: 对比 H2 核心组件与计算机科学经典概念**
...
**MVMap COW B-Tree（mvstore/MVMap.java + Page.java）**：这是 H2 存储引擎最核心的数据结构。`MVMap` 提供了类似 `java.util.NavigableMap` 的接口，但底层实现是支持并发的持久化 COW B-Tree。通过 `get()` 方法可以理解 B-Tree 的查找过程——从根节点开始，在内部节点中通过二分查找找到下一层，直到叶子节点。`put()` 方法则展示了 COW 的写入过程——复制修改路径上的每个节点。这些代码帮助深入理解《算法导论》中 B-Tree 的插入、删除、分裂、合并等操作在工程实践中如何实现。
...
如图 12-11 所示，根据你的实际需求，可以选择以下四条路径之一作为切入点。每条路径都标注了入口类、核心文件和预期时间。

**图 12-11: 罗列 H2 源码四条研读路径的入口与目标**
...
如图 12-12 所示，在 IDE 中设置断点并逐步追踪执行流程，是理解源码最高效的方式之一。以下是三个典型的调试场景，每个场景都标注了关键的断点位置、预期观察对象和执行路径。

**图 12-12: 追踪三大调试场景的端到端调用链路**
...
建议首次在 IntelliJ 中打开 H2 项目后，从 `Tokenizer.java`（词法分析）经 `MVMap.java`（B-Tree 接口）、`MVStore.java`（存储引擎提交路径）到 `TransactionMap.java`（事务分界）依次阅读 Javadoc，建立整体印象后再选择 12.4.2 中的调试场景逐步追踪。
...
第 12 章把 H2 的设计哲学、横向对比与多方向研读路径整合在一起，下列四题用来检验读者能否把"为什么这样选"转化为可对比、可复用的工程判断。

**1. 🟢★ 用一句话概括 H2 "小而精"的设计哲学，并指出它在源码层面有哪两处最直观的体现。**

> 提示：从代码量、抽象层数、模块边界等维度切入；可参考 §12.1 的四项权衡。
> 回顾：§12.1、§12.3

**2. 🟢★ 在嵌入式数据库定位上，H2 与 SQLite 最关键的差异是什么？请从 §12.2 的特性表中至少选两个维度作答。**

> 提示：聚焦语言运行时、并发模型与 SQL 兼容广度。
> 回顾：§12.2

**3. 🔵★★ 假如让 H2 改走"大而全"路线，最先需要牺牲的是哪个特性？为什么？**

> 提示：从 Single Writer CAS 的扩展性瓶颈、嵌入式部署对体积/依赖的约束反推。
> 回顾：§12.1.3、§12.2

**4. 🟠★★★ 在 §12.4 的研读工具与练习清单中任选两个工具或练习，跑一遍并记录工具的实际输出与练习的可观测现象。**

> 提示：推荐"IntelliJ Evaluate Expression + 练习 3 追踪 B-Tree 页分裂"组合;只需记录关键的几行截图或日志。
> 回顾：§12.4.3、§12.4.4

## 12.7 延展阅读

- 官方完整特性列表：H2 官方文档《Features》(`h2/src/docsrc/html/features.html#feature_list`)
- 安全特性完整说明：H2 官方文档《Security》(`h2/src/docsrc/html/security.html`)
- 快速入门与命令行指南：H2 官方文档《Tutorial》(`h2/src/docsrc/html/tutorial.html`)
- H2 官方文档《Advanced》(`h2/src/docsrc/html/advanced.html#sql_injection`) — SQL 注入防护配置
- 本书第1章§1.1 — H2 核心特性对比总览
