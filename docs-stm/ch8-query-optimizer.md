...
掌握查询优化器的三种连接策略、代价模型与索引选择算法后，下列四题帮助读者把"代价驱动 + 启发式搜索"的核心思路落到具体场景判断上。

**1. 🟢★ 描述 `PlanItem.cost` 在 `Plan.calculateCost()` 中的累计公式，并指出三项加权和分别衡量哪一类开销。**

> 提示：注意复合乘法 `cost = cost + cost × item_cost` 中前后两个 `cost` 的语义差异，再回到 `Index.getCost()` 的 rowCount/lookup/condition 三段加权。
> 回顾：§8.3 代价模型

**2. 🟢★ 列出 `IndexCondition` 中 EQUALITY、RANGE、SPATIAL 三种类型分别能匹配哪些索引实现，并解释为何 `MultiVersionIndex` 不被空间条件直接命中。**

> 提示：从掩码匹配 `getMask()` 与 `IndexType` 的组合关系入手，再看 `getBestPlanItem()` 如何把不可用条件折算成无穷代价。
> 回顾：§8.4 索引选择机制

**3. 🔵★★ 估算 N=5、N=7、N=10 表参与 JOIN 时暴力枚举的搜索空间规模差异，并说明为何 H2 选择在 `≥8` 表时切换到 Hybrid 策略。**

> 提示：从 N! 阶乘增长曲线出发，结合 `Constants.MAX_TABLES_PER_QUERY_BRUTE_FORCE` 的阈值判断；再思考贪心 + 遗传组合的渐进时间复杂度。
> 回顾：§8.2 三种连接顺序策略、§8.6 优化实践建议

**4. 🟠★★★ 设计一条 7 张表参与 JOIN 的 SQL 与 schema，使其代价空间在表数从 7 增至 8 时刚好触发暴力枚举到 Hybrid（贪心+遗传）策略的切换；用 `EXPLAIN ANALYZE` 对比两种规划下的代价输出。**

> 提示：从 `Optimizer.optimize()` 的入口分支入手，关注 7→8 表临界值前后 `chooseBruteForce()` 与 `calculateBruteForce/Greedy/Genetic()` 的进入条件；可借助 `SET QUERY_STATISTICS` 观察实际计划。
> 回顾：§8.1 Optimizer 类架构、§8.2 三种连接顺序策略、§8.7 ASCII 优化流程图
