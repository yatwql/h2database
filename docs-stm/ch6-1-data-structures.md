...
Page 的生命周期中的两种状态:
  ...
  Page.copy() 的实现 (Page.java:380-385):
    1. 创建新 Page 实例
...
如图 6-1 所示，本章深入分析了 H2 数据库中三个核心数据结构与版本管理算法：

- **B-Tree 索引结构**（6.1）：MVMap 基于 B+Tree 变体，采用 COW 页面管理。内部节点仅存储分隔键，叶子节点存储实际键值对，通过二分查找实现 O(logₖ N) 的查找效率。默认 keysPerPage 为 48，树深度通常不超过 4 层，在 99% 的情况下 B-Tree 的 4 层访问全部在内存中完成。
- **Copy-on-Write 版本管理**（6.2）：不修改现有不可变对象，通过创建副本并在副本上修改然后以 CAS 原子替换根引用实现无锁并发。写放大因子等于 B-Tree 深度（通常 3-4×），但顺序 I/O 特性以及批量提交可显著降低有效写放大。该机制是 MVStore 无锁读和天然快照支持的技术基础。
- **MVCC 多版本控制**（6.3）：通过 `VersionedValue` 包装器在值中嵌入版本信息（operationId 编码事务 ID 和日志 ID），结合 undo log 实现读写互不阻塞的快照隔离。读路径无需查 undo log——`committedValue` 提供已提交版本的一步访问，写路径仅记录增量 `oldValue` 用于回滚。

以上内容构成第 6 章的**数据结构基础篇**：先说明页、树和版本如何在内存中组织，再为后续存储算法篇理解 Chunk、缓存、空闲空间和页面平衡机制建立共同语境。
