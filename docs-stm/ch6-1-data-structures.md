...
**图 6-6: 追踪 RootReference 的 CAS 原子更新序列**
...
**图 6-7: 对比 MVStore 无锁读路径与 CAS 写路径**
...
```
**图 6-10: 追踪 COW 临时对象在 GC 各代的回收时序**
```text
...
**图 6-15: 拆解 VersionedValue 的字段布局与状态转换**
...
```
**图 6-19: 描绘 TxDecisionMaker 冲突检测与解决路径**
```text
...
下面 4 道自查题用来检验读者能否把本章三种基础数据结构的关键决策迁移到具体场景。

**1. 🟢★ 描述 B-Tree 节点分裂的触发条件，并说明分裂后父节点的子指针数组如何递归更新。**

> 提示：关注节点容量阈值与中间键上提；当父节点同样越界时，分裂如何沿路径继续向上传递。
> 回顾：§6.1、§6.1.3

**2. 🟢★ 对比"Copy-on-Write"与"原地更新"在并发安全性上的差异，并解释为何 MVStore 的无锁读依赖前者。**

> 提示：从读者是否会观察到中间状态、写者是否需要全局锁两个角度展开；联系版本根指针的原子切换。
> 回顾：§6.2、§6.2.2

**3. 🔵★★ 假设把 H2 B-Tree 替换为红黑树作为索引核心，索引的随机查找与范围扫描性能将分别损失什么收益？**

> 提示：考虑磁盘/页面访问的扇出、节点内二分查找的局部性，以及叶子层是否天然有序。
> 回顾：§6.1、§6.1.2

**4. 🟠★★★ 在 IDE 中给 `Page.binarySearch` 设置断点，跟踪一次主键范围查询的节点访问轨迹，记录每一层加载的页面 ID 与命中的子指针下标。**

> 提示：使用 H2 测试目录下的 `TestMVStore` 构造一个百万行表；断点触发后观察调用栈中 `Cursor` 的下钻路径。
> 回顾：§6.1.2、§6.1.4
