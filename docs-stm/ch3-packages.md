...
如图 3-1 所示，H2 Database 的源码根包为 `org.h2`，共包含约 843 个 Java 源文件，分布在 25 个一级子包和约 15 个二级子包中。从依赖关系的角度看，这些包构成了一个**有向无环分层图**，上层依赖下层，下层绝不反向依赖上层。

**图 3-1: 展示 org.h2 顶层包的分层依赖结构**
...
如图 3-3 所示，从矩阵可以清晰看出：`value` 包处于依赖链最底层，几乎所有上层包都引用它；`engine` 包是核心枢纽，依赖关系和被依赖关系最为密集；`store.fs` 位于层级最底层，仅 `store` 包依赖它。这种严格的单向依赖结构是 H2 保持长期可维护性的关键设计决策。

**图 3-3: 勾勒 H2 五层体系的分层视图**
...
下面三张图按"静态结构 → 实例生命周期 → 会话内部状态"逐层展开 engine 包：图 3-6 梳理核心类的引用关系，图 3-7 刻画 Database 实例的状态机，图 3-8 拆解 SessionLocal 的事务协调字段。

**图 3-6: 梳理 engine 包内核心类的引用关系**
...
如图 3-7 所示，如图 3-6 所示，`Engine` 是单例入口，按数据库名称查找或创建 `Database` 实例。`Database` 是整个系统的上帝对象（God Object），几乎所有全局状态的访问都要经过它。`SessionLocal` 是每次 SQL 执行的上下文容器，负责管理事务隔离级别、锁持有和本地临时数据。

**图 3-7: 刻画 Database 实例的生命周期状态转换**
...
**图 3-8: 拆解 SessionLocal 的状态字段与事务协调职责**
...
以下四张图把 SQL 解析与执行的全链路拆成四个视角：图 3-9 给出从词法到执行的整体阶段，图 3-10 展开 Parser 的命令派发层次，图 3-11 对比 DML、DDL 与 Query 的执行路径，图 3-12 揭示 commandMap 缓存与重新编译的判定规则。

**图 3-9: 串联 SQL 从解析到执行的核心阶段**
...
如图 3-11 所示，如图 3-10 所示，每个 `parseXxx()` 方法的内部实现遵循相同的模式：先调用 `readIf()` 或 `readEnum()` 检查当前 token 的关键字，然后递归调用子解析器读取表名、列名、表达式等子结构，最后构造对应的 `Prepared` 子类实例。这种设计使解析逻辑与命令逻辑完全分离——解析器负责"读懂"SQL，而 `Prepared` 子类负责"执行"语义。

**图 3-11: 对比 DML、DDL 与 Query 的执行路径**

```text
如图 3-12 所示，DML (Insert/Update/Delete)     DDL (Create/Alter/Drop)           Query (Select)
─────────────────────────────  ────────────────────────────      ─────────────────────────
...
```
**图 3-12: 拆解 commandMap 缓存与重新编译的判定路径**
...
**图 3-13: 概览 expression 子包的目录划分与代表类**
...
如图 3-14 所示，如图 3-13 所示，表达式系统的设计核心是 `Expression` 的**三层接口**：`mapColumns()` 在编译期绑定列引用，`optimize()` 执行常量折叠和类型推导，`getValue()` 在运行期递归求值。这一模式使得表达式树可以被独立优化和执行。

**图 3-14: 演示 Expression 自底向上求值与类型提升**
...
下面三张图围绕 Table 抽象展开三个层面：图 3-15 梳理 Table 的派生体系，图 3-16 追踪 TableFilter 嵌套连接的执行时序，图 3-17 对比四种连接策略的代价特征。

**图 3-15: 梳理 Table 抽象基类的派生体系**
...
如图 3-16 所示，TableFilter ──→ PlanItem ──→ Plan (查询计划)
```

**图 3-16: 追踪 TableFilter 嵌套连接的执行时序**
...
如图 3-18 所示，Table 通过 `getIndexes()` 获取其索引列表，`TableFilter` 在 `findBestIndex()` 中根据查询条件选择代价最小的索引。`IndexCondition` 描述了索引可以加速的谓词类型，优化器利用这些信息来裁剪扫描范围。

**图 3-18: 勾勒 findBestIndex 选择扫描策略的决策树**
...
如图 3-19 所示，如图 3-18 所示，索引扫描策略的选择直接影响查询性能。决策树的核心逻辑在 `TableFilter.findBestIndex()` 中实现：它遍历表的所有索引，对每个索引计算 `IndexCondition` 的匹配度，估算扫描行数，选择代价最小的索引。精准查找（等值条件匹配索引前缀）的代价最低，范围扫描次之，全表扫描最差。二级索引还需要额外考虑"回表"的开销——即通过索引键找到主键值后，再通过主键索引获取完整行数据。

**图 3-19: 对比 主键索引与二级索引的数据流向**
...
下面四张图按"写入路径 → 数据结构 → 物理布局 → 提交时序"串联 MVStore 的存储机制：图 3-20 追踪 MVMap.put 的写入链路，图 3-21 演示 B-Tree 的页分裂规则，图 3-22 拆解 .mv.db 文件中 chunk 的物理布局，图 3-23 刻画 Checkpoint 与后台 writer 的时序协作。

**图 3-20: 追踪 MVMap.put 触发的写入与持久化链路**
...
如图 3-21 所示，如图 3-20 所示，`MVStore` 的核心设计理念是**无锁读取 + COW 写入**：读操作通过 `RootReference` 获取稳定的 B-Tree 根节点引用，写操作创建新的节点路径，然后通过 CAS 原子更新根引用。这使得读操作永远不会被写操作阻塞。

**图 3-21: 演示 MVStore B-Tree 结构与页分裂规则**
...
如图 3-22 所示，如图 3-21 所示，B-Tree 是 MVStore 的核心数据结构。每个节点（Page）可以包含最多 192 个子节点或键值对。当叶子页写满时触发页分裂——将一半的键值对移动到新页，并在父节点中插入分隔键。页分裂可能向上传播，甚至导致根节点分裂，从而使树的高度增加一级。COW（Copy-on-Write）机制确保分裂过程中正在进行的读操作不会看到不一致的状态：写操作创建新的节点路径，通过 CAS 原子切换根引用后，旧版本对并发读仍然可见。

**图 3-22: 拆解 .mv.db 文件中 chunk 的物理布局**
...
如图 3-23 所示，如图 3-22 所示，Chunk 是 MVStore 的物理写入单元。所有写操作都是"追加写入"（append-only）的——新的页数据总是写入新的 chunk，compaction 之前不会覆写旧的 chunk。这种设计有两个重要优势：一是写操作是顺序 I/O，比随机 I/O 快得多；二是数据库崩溃时，最多丢失最后一个未完成的 chunk，不会产生部分写入导致的损坏。`FreeSpaceBitSet` 负责跟踪哪些 chunk 已被回收，供后续写入复用。

**图 3-23: 追踪 Checkpoint 与后台写线程的提交时序**
...
如图 3-23 所示，MVStore 的写入策略是"延迟批量写入"：每次 `put()` 操作先修改内存中的 B-Tree，在 `commit()` 时序列化脏页面到新 chunk 保证持久性，然后在后台线程中定期执行 checkpoint 将完整状态写入新的 chunk。`MetaRoot` 指针在 checkpoint 时原子更新，确保数据库重启后能从最近的 checkpoint 快速恢复。未提交的变更在崩溃后不会丢失，因为未持久化的变更会在重启时丢弃。
...
如图 3-25 所示，如图 3-24 所示，`MVPrimaryIndex` 使用行 ID（long 类型）作为键、完整行数据作为值。`MVSecondaryIndex` 从索引键映射到主键值（即行 ID），因此需要通过主键索引的二次查找才能获取完整行。这是标准的**索引组织表 + 二级索引回表**模式。

**图 3-25: 追踪 SQL INSERT 到 MVMap 的完整映射链**
...
下面三张图分别从可见性、状态机与冲突检测三个角度刻画 mvstore.tx 包：图 3-26 拆解 TransactionMap 的版本可见性规则，图 3-27 刻画 Transaction 对象的状态转换路径，图 3-28 演示 TxDecisionMaker 对并发写写冲突的检测。

**图 3-26: 拆解 TransactionMap 版本可见性的判断规则**
...
如图 3-27 所示，如图 3-26 所示，该事务模型实现了 **SI（Snapshot Isolation，快照隔离）**，在此基础上通过 `TxDecisionMaker` 检测写冲突，可升级到可串行化隔离级别。

**图 3-27: 刻画 Transaction 事务对象的状态转换路径**
...
**图 3-28: 演示 TxDecisionMaker 并发写写冲突的检测**
...
如图 3-29 所示，该包只有一个类，实现了 LIRS 缓存算法。LIRS 的核心思想是区分"热数据"和"冷数据"的访问模式，即使冷数据频繁出现（如全表扫描），也不会把热数据挤出缓存。

**图 3-29: 拆解 CacheLongKeyLIRS 三个集合的内部组织**
...
**图 3-30: 对比 LRU 与 LIRS 在扫描场景下的命中差异**
...
LIRS 算法通过区分"内引用间隔"和"外引用间隔"来判断数据是热还是冷。全表扫描发生时，扫描到的数据会"污染" LRU 缓存，挤出热数据。而 LIRS 将频繁访问的数据标记为 Low Inter-reference Recency（LIR），将其固定在缓存中。即使大量冷数据到来，也不会淘汰 LIR 集合。这是 H2 在全表扫描场景下仍能保持良好缓存命中率的关键。`CacheLongKeyLIRS.java` 的实现使用了两个主要数据结构：一个 LIR 栈（Stack）记录所有缓存项的访问顺序，和一个 HIR 队列（Queue）管理冷数据。其核心方法 `get()` 和 `put()` 在 `CacheLongKeyLIRS.java:280-380` 实现了完整的 LIRS 算法逻辑。
...
如图 3-31 所示，MVRTreeMap 是一棵支持动态插入/删除的 R-Tree，用于加速几何数据类型（如 `Geometry`）的空间查询（包含、相交、距离等）。

**图 3-31: 梳理 MVRTreeMap 空间索引的层级组织**
...
如图 3-32 所示，如图 3-31 所示，R-Tree 是 B-Tree 在多维空间上的推广。每个节点维护一个最小边界矩形（MBR, Minimum Bounding Rectangle），该矩形包围了其子节点所覆盖的所有空间数据。查询时，从根节点开始递归检查 MBR 是否与查询区域相交——如果不相交，则整个子树都可以跳过，从而实现空间剪枝。R-Tree 在 GIS（地理信息系统）和空间数据库中被广泛使用，H2 通过 `MVRTreeMap` 实现了对 `Geometry` 类型的高效空间查询支持。

**图 3-32: 演示 R-Tree 范围查询的 MBR 剪枝路径**
...
如图 3-32 所示，该图展示了 R-Tree 在执行空间范围查询时的剪枝过程。查询从根节点开始，递归检查每个节点的 MBR 是否与查询区域相交——如果不相交，则该节点和整棵子树都可以跳过，无需进一步搜索。在上述示例中，内部节点 2 的 MBR 完全在查询区域之外，因此该节点下的所有叶子节点都被直接跳过，减少了约 50% 的节点访问量。在大型空间数据集上，这种 MBR 剪枝通常可以将搜索范围缩小到整体数据的 10%~40%。
...
如图 3-36 所示，如图 3-35 所示，所有服务器在接收到请求后，最终都在本地创建 `SessionLocal` 实例，走与嵌入式模式完全相同的引擎路径。这使得服务器模式几乎没有额外的性能开销。

**图 3-36: 追踪 TcpServer 线程模型与请求处理时序**
...
**图 3-37: 演示 Value.convertTo 触发的类型自动转换**
...
如图 3-38 所示，如图 3-37 所示，`Value` 对象是**不可变的**——所有转换操作返回新实例。这一设计简化了表达式求值中的引用管理和并发安全。

**图 3-38: 梳理 Value 抽象基类的子类分类体系**
...
**图 3-40: 追踪 LocalResult 结果集的构造与迭代路径**
...
**图 3-41: 串联 H2 密码到 AES 文件加密的完整链路**
...
如图 3-42 所示，如图 3-41 所示，加密对上层完全透明。`FilePathEncrypt` 作为 `FilePath` 的装饰器，在 I/O 通道层面完成加解密，存储层和引擎层无需任何修改。

**图 3-42: 拆解 H2 认证、授权与静态加密三层体系**
...
如图 3-42 所示，安全架构分为三层：认证层验证用户身份，授权层控制数据访问权限，加密层保护静态数据。认证使用 SHA-256 加盐哈希存储密码，防止彩虹表攻击。加密使用 AES/CBC 模式，每个 128 位块独立加密，密钥通过用户提供的密码短语派生。`FilePathEncrypt` 作为装饰器位于 I/O 栈中，加解密过程对上层完全透明——存储层和引擎层感知不到加密的存在。`Authenticator` 采用策略模式，允许用户自定义认证逻辑（如集成 LDAP 或 OAuth）。
...
如图 3-44 所示，如图 3-43 所示，`Recover.java` 和 `DirectRecover.java` 是 H2 数据库运维中最关键的工具。当数据库文件因系统崩溃或硬件故障而损坏时，`Recover` 可以直接扫描 MVStore 文件的 chunk 结构，提取所有可读的页数据，并生成 SQL 恢复脚本。这一工具使用了 mvstore 的内部 API 来直接读取 chunk、解析页结构和反序列化行数据，绕过了 SQL 引擎层和事务层。`Script` 和 `RunScript` 则用于逻辑备份与恢复——它们导出的是纯 SQL 语句，可以在不同版本的 H2 之间迁移数据。

**图 3-44: 追踪 Recover 工具从损坏文件重建数据的步骤**
...
如图 3-44 所示，Recover 的数据恢复流程是一条五步流水线：扫描文件头获取 chunk 索引表，然后逐个读取 chunk 并验证校验和，接着反序列化页数据为行记录，再根据元数据生成 DDL 和 DML 脚本，最后用户执行脚本完成重建。绕过引擎层设计的好处是：即使引擎层元数据损坏（如系统表无法打开），Recover 仍然可以直接从文件结构层提取数据。
...
如图 3-46 所示，**层定位**: 为 MVStore 页面压缩提供可插拔的压缩算法实现，在持久化路径中透明工作。

**图 3-46: 标注 页面压缩在 MVStore 写入路径中的位置**
...
如图 3-47 所示，`CompressDeflate` 则使用 Java 标准库的 `java.util.zip.Deflater`，提供更高的压缩率但 CPU 开销显著增加。适用于存储空间敏感但读取频率较低的归档场景。

**图 3-47: 对比 LZF 与 Deflate 压缩算法的核心特性**
...
如图 3-48 所示，**层定位**: 提供多数据库方言兼容层，使 H2 能模拟 MySQL、Oracle、PostgreSQL 等数据库的 SQL 语法和函数行为。

**图 3-48: 拆解 SQL 兼容模式的语法开关与函数注册**
...
如图 3-49 所示，当用户执行 `SET MODE MySQL` 后，`Mode.java` 中的规则控制语法差异，而 `FunctionsMySQL` 等类提供特有的函数定义。`Mode.java` 位于 `org.h2.engine` 包中（而非 mode 包），定义了大量布尔开关如 `mode.nullConcatIsNull`（MySQL 的 `NULL || 'x'` 行为）、`mode.serialColumn`（Oracle 的序列语法）等。mode 包中的 `Functions*` 类通过 `Function.addFunction()` 在启动时注册到全局函数表，解析器根据当前 Mode 在函数名查找时匹配对应的实现类。

**图 3-49: 追踪 兼容模式函数注册到运行时解析的链路**
...
如图 3-52 所示，每一层都只依赖其正下方的层，没有跳层或反向依赖。这种严格的**分层依赖**是 H2 代码可维护性的基石——替换存储引擎（PageStore → MVStore）对上层的影响被控制在 mvstore.db 的桥接层内。

**图 3-52: 归纳 H2 各核心包采用的设计模式分布**
...
本章梳理了 H2 各核心包的职责切分与依赖走向，下列题目帮助读者把"包级地图"转化为可应用的判断力。

**1. 🟢★ 用一句话概括 `org.h2.command` 与 `org.h2.expression` 的职责差异，并说明二者在调用链上的先后顺序。**

> 提示：从 SQL 文本到执行计划的处理阶段考虑，谁负责"语句"，谁负责"求值"。
> 回顾：§3.4 Command 层、§3.5 Expression 层

**2. 🟢★ 列出 `Value` 类型族（如 ValueInteger / ValueVarchar / ValueArray）的共同抽象基类，并指出该基类承担的核心职责。**

> 提示：关注类型转换、比较与序列化三类操作，留意抽象方法签名所暗示的契约。
> 回顾：§3.10 类型与结果

**3. 🔵★★ 如果将 `org.h2.table` 与 `org.h2.index` 合并为单一包，会破坏哪些既有设计假设？请至少给出两条结构性理由。**

> 提示：从抽象层级、依赖方向、可替换的索引实现三方面切入。
> 回顾：§3.6 Table/Index 层、§3.12 包间依赖总结

**4. 🟠★★★ 在本地构建 H2 主 jar 后，用 `jdeps -summary` 输出包级依赖图，对照本章 §3.1 的依赖论述，找出至少一条本章未显式标注的依赖边。**

> 提示：聚焦 mvstore、store、value 等底层包被哪些上层包引用，注意排除测试包。
> 回顾：§3.1 包全景图、§3.12 包间依赖总结
