...
**图 A-1: 串联 SELECT 从 JDBC 入口到 FileStore 的八泳道执行轨迹**
...
**图 A-5: 拆解 Page Pointer 64 位字段：chunkId、offset、length、type**
...
```text
   ...
   └─────────────────────────────────────────────────────────────────┘
```
**图 A-12: 展示 TransactionStore 三主状态迁移与回滚分支**
...
本案例至此结束。下一节 A.4（如有）将把案例 A、B、C 三条路径在同一张全景图上拼接，给出 H2 在"读—写—恢复"三个维度上的完整 ACID 落地视图。
