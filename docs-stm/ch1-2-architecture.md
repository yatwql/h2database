...
理解 H2 的整体定位与八层骨架后，下面四题帮助读者把"小而全"的设计哲学转化为可对照、可验证的工程判断。

**1. 🟢★ 与 SQLite、Derby、HSQLDB 相比，H2 的"纯 Java + 多模式 + 多协议"组合带来了哪些独特优势与代价？**

> 提示：从语言运行时、部署形态、协议兼容三个维度对照特性表权衡。
> 回顾：§1.1、§1.3

**2. 🟢★ 复述 H2 八层架构中各层的职责与依赖方向，并指出哪些层属于同一个逻辑组。**

> 提示：注意接入层、SQL 处理层、存储抽象层都是由两个子层聚合而成。
> 回顾：§1.2、§1.4

**3. 🔵★★ MVStore 取代 PageStore 的根本设计动因是什么？为什么这种替换可以做到上层 90%+ 代码不变？**

> 提示：从并发模型、写入路径、崩溃恢复三方面分析，再回到接口隔离原则。
> 回顾：§1.1、§1.3

**4. 🟠★★★ 用 IDE 在 `org.h2.Driver` 上设置断点，跟踪一次 `Class.forName("org.h2.Driver")` 加载到 `JdbcConnection` 返回的完整调用链。**

> 提示：观察 SPI 注册、`Engine.openSession`、`Database.connectSession` 的依次出现顺序。
> 回顾：§1.2、§1.3
...
把八层模型从图示理解推进到工程判断，下面四题分别针对接口边界、模块定位、依赖不变量和实证验证。

**1. 🟢★ 接入层与引擎层之间由哪些接口划定边界？请举出 JDBC 子层和 Server 子层各自的关键入口。**

> 提示：聚焦 `CommandInterface`、`SessionLocal` 与 `Service` 三个接口的角色。
> 回顾：§2.1、§2.3

**2. 🟢★ 在分层视图中，`SessionLocal` 与 `SessionRemote` 各自处于哪一层?它们在 C/S 模式下如何分工?**

> 提示：注意 `SessionRemote` 位于 `org.h2.engine`,但只在客户端运行。
> 回顾：§2.1、§2.4

**3. 🔵★★ 假设把 Engine 层的功能下沉到 JDBC 层（让 `JdbcConnection` 直接持有 Database），会破坏哪些不变量？**

> 提示：从依赖方向、生命周期管理、Server 子层的复用性三个角度推演。
> 回顾：§2.1、§2.2

**4. 🟠★★★ 用 `jdeps` 分析 `h2-*.jar`，将其输出与图 2-17 的层间依赖图对照，找出任何未在图中体现的依赖。**

> 提示：使用 `jdeps -v -e "org\.h2\..*" h2.jar` 并按包过滤，关注横向与跨层引用。
> 回顾：§2.2、§2.4
