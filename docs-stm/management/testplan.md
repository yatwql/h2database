...
EPUB 不接入 `final_check.py`（按需交付物均走人工视觉验证起步，与印刷级 PDF 一致）。
...
| v6.1 | EPUB 视觉验证 | 人工 | 人工 | pandoc 引擎；EPUB 由 cover.md 提取元数据；不接入 `final_check.py` |
