...
本章将深入剖析 MVStore 持久化引擎的架构设计与实现细节。9.1 节概述 MVStore 的总体架构、生命周期和版本控制机制；9.2 节详述 B-Tree 与 Page 的内部结构和序列化格式；9.3 节解析 Chunk 的文件布局与空间分配策略；9.4 节说明 Undo Log 机制及崩溃安全保障；9.5 节介绍检查点触发逻辑与后台写入线程。第10章将在此基础上进一步讨论锁与并发控制机制。
...
如图 9-5 所示，`unsavedMemory` 计数器在 `MVStore.java` 第 1198-1215 行的 `accountForWrittenPage()` 中更新。每当一个 Page 被修改时，其内存占用量被加到 `unsavedMemory` 上。当 `isSaveNeeded()` 返回 true 时（第 1418-1424 行），后台线程被唤醒执行 store 操作。
...
如图 9-6 所示，版本控制的核心作用是支持 MVCC 快照读取。当一个事务开始时，它记录当前的 `currentVersion`。在整个事务期间，它读取的 B-Tree 页面都是基于这个版本（或更早版本的）快照。即使其他事务提交了新的修改（创建了新的 RootReference），当前事务仍然可以看到一致的旧版本数据，因为旧版本的 RootReference 仍然保留在版本链中。
...
`pos` 字段使用 `AtomicLongFieldUpdater` 进行更新（第 89-91 行），这是为了在页面被保存线程写入位置信息的同时，其他线程可能并发地将页面标记为"已删除"。这种无锁并发更新在 MVStore 中非常典型。
...
2. 读取 `check` 短整数，计算校验值 `chunkId ^ offset ^ pageLength`，对比校验
3. 读取 `pageNo`、`mapId`、`keyCount`、`type`
4. 如果是压缩页面（`type & PAGE_COMPRESSED`），使用 LZF 或 Deflate 解压
5. 调用 `map.getKeyType().read()` 读取键数组
...
如图 9-14 所示，MVStore 没有传统数据库中的 Buffer Pool 页面置换机制（如 LRU）。相反，它依赖于 Java GC 和不可变对象的特性。当一个页面被更新的版本替代后，旧版本如果没有被任何事务的快照引用，就会被 GC 自动回收。这种设计简化了代码，但依赖于 JVM 的内存管理和 GC 性能。
...
如图 9-23 所示，MVStore 的检查点机制比传统数据库简单得多——它本质上是将内存中的脏页面写入一个新 Chunk。
...
压缩发生在 `Page.write()` 序列化之后，对整个键值数据区域进行压缩：
...
`TransactionMap` 是 `MVMap` 的事务性包装器，提供了基于 MVCC 的行级并发控制。它通过 `VersionedValue` 为每个键值对维护多个版本，支持快照隔离。
...
每个 MVMap 的值实际上是一个 `VersionedValue<V>`，包含：
...
如图 10-11 所示，被牺牲的 session 会释放它已持有的所有锁（通过异常传播，最终由上层事务管理代码回滚），使得死锁链被打破，其他 session 可以继续执行。
...
MVTable 的死锁检测算法本质上是 DFS 在锁等待图上的遍历。等待图的节点是 session，边从等待者指向持有者。以下展示了更复杂的场景：
...
如图 10-13 所示，当检测到死锁时，被选为牺牲的 session 需要释放所有已持有的锁，让其他 session 可以继续执行：
...
如图 10-25 所示，看门狗的核心职责是周期性地重写锁文件（更新心跳时间戳）。如果拥有锁的进程崩溃，锁文件不会被更新，其他进程在等待一段时间后可以检测到锁已过期，从而接管。
...
如图 10-28 所示，两种锁模式有不同的适用场景和可靠性特征。以下从多个维度进行对比：
...
- 死锁检测算法利用全局事务依赖图进行深度优先遍历，以事务超时机制为辅助，在保障正确性的同时将运行时开销降至最低。
